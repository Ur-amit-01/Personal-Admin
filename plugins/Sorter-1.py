...
async def media_with_caption(client, message):
    ...
    # Plain text is used for title extraction (structured "Title »" parsing).
    caption_plain = message.caption or ""

    # .html reconstructs the caption WITH hidden/rich-text hyperlinks rendered as
    # <a href="..."> tags (Telegram stores these as caption_entities, not in the
    # raw string) — required so extract_youtube_link/extract_notes_link can see them.
    caption_html = message.caption.html if message.caption else ""

    title = extract_title(caption_plain)
    youtube_link = extract_youtube_link(caption_html) or extract_youtube_link(caption_plain)
    notes_link = extract_notes_link(caption_html) or extract_notes_link(caption_plain)
...
# FINAL SEND
# =====================================================================
async def resolve_thumb(client, session):
    """
    Pyrogram's `thumb` param only accepts a local file path or a BytesIO object —
    it does NOT accept a Telegram file_id directly. If the user uploaded a custom
    thumbnail, we only stored its file_id, so we must download it in-memory first.
    """
    # Default thumbnail: already a local path, use as-is
    if session.get("thumb_path"):
        return session["thumb_path"]

    # Uploaded thumbnail: stored as file_id, must be downloaded first
    if session.get("thumb_file_id"):
        try:
            thumb_bytes = await client.download_media(session["thumb_file_id"], in_memory=True)
            if thumb_bytes:
                thumb_bytes.name = "thumbnail.jpg"  # Pyrogram needs a filename/extension hint
                return thumb_bytes
        except Exception:
            return None  # fall through to no-thumb rather than crashing
...
async def send_final_media(client, chat_id, session, caption):
    """
    IMPORTANT: Telegram's Bot API ignores the `thumb` parameter whenever the media
    is sent by re-using an existing file_id (thumb only applies to fresh multipart
    uploads). So to make our custom thumbnail actually show up, we must download
    the media once and re-upload it as a new file — not just pass file_id through.
    """
    thumb = await resolve_thumb(client, session)
    local_media_path = None

    try:
        # Download the actual video/document fresh so the re-upload is a real
        # multipart upload (required for `thumb` to take effect).
        local_media_path = await client.download_media(session["file_id"])
        if not local_media_path:
            raise RuntimeError("Failed to download original media for re-upload")

        if session["media_type"] == "video":
            await client.send_video(chat_id, local_media_path, caption=caption, thumb=thumb, parse_mode=ParseMode.HTML)
        else:
            await client.send_document(chat_id, local_media_path, caption=caption, thumb=thumb, parse_mode=ParseMode.HTML)
        ...
        try:
            ...
            source = local_media_path or session["file_id"]  # fall back to file_id if download itself failed
            if session["media_type"] == "video":
                await client.send_video(chat_id, source, caption=caption, parse_mode=ParseMode.HTML)
            else:
                await client.send_document(chat_id, source, caption=caption, parse_mode=ParseMode.HTML)
        ...
        # Always clean up the downloaded temp file so disk usage doesn't creep up
...
async def teacher_selected(client, callback_query: CallbackQuery):
    ...
    # Download+reupload can take a while for large videos — let the user know it's working
    status_msg = None
    try:
        status_msg = await client.send_message(callback_query.message.chat.id, "**⏳ Processing... please wait**")
    except RPCError:
        pass
