...
async def send_final_media(client, chat_id, session, caption):
    ...
    try:
        if session["media_type"] == "video":
            await client.send_video(chat_id, session["file_id"], caption=caption, thumb=thumb, parse_mode="html")
        else:
            await client.send_document(chat_id, session["file_id"], caption=caption, thumb=thumb, parse_mode="html")
        ...
        try:
            ...
            if session["media_type"] == "video":
                await client.send_video(chat_id, session["file_id"], caption=caption, parse_mode="html")
            else:
                await client.send_document(chat_id, session["file_id"], caption=caption, parse_mode="html")
